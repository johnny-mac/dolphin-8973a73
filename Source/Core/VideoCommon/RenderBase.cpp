...
void Renderer::DrawDebugText()
{
  ...
  if (g_ActiveConfig.bShowFPS)
  {
    ...
    ImGui::SetNextWindowPos(ImVec2(ImGui::GetIO().DisplaySize.x - (10.0f * scale), 10.0f * scale),
                            ImGuiCond_Always, ImVec2(1.0f, 0.0f));
    ImGui::SetNextWindowSize(ImVec2(100.0f * scale, 30.0f * scale));
    ...
    {
      ImGui::Text("FPS: %.2f", m_fps_counter.GetFPS());
    }
    ImGui::End();
  }
  ...
  if (show_movie_window)
  {
    ...
    ImGui::SetNextWindowPos(ImVec2(ImGui::GetIO().DisplaySize.x - (10.0f * scale), 50.0f * scale),
                            ImGuiCond_FirstUseEver, ImVec2(1.0f, 0.0f));
    ImGui::SetNextWindowSizeConstraints(ImVec2(150.0f * scale, 20.0f * scale),
  ...
  {
    // TODO: Move vertex loaders to their own option.
    Statistics::Display();
    VertexLoaderManager::DisplayLoaders();
  }
...
static std::string GenerateImGuiVertexShader()
{
  ...
  ss << "  " << (api_type == APIType::D3D ? "out_pos" : "gl_Position")
     << "= float4(rawpos.x * u_rcp_viewport_size_mul2.x - 1.0,"
...
void Renderer::RenderImGui()
{
  ...
  // TODO: Support index buffers in utility draws.
  ...
  for (int i = 0; i < draw_data->CmdListsCount; i++)
  {
    ...
    for (const ImDrawCmd& cmd : cmdlist->CmdBuffer)
    {
      ...
      DrawUtilityPipeline(&ubo, sizeof(ubo), &cmdlist->VtxBuffer[vertex_base], sizeof(ImDrawVert),
                          cmd.ElemCount);
      vertex_base += cmd.ElemCount;
    }
  }
}
                    ...
                    u64 ticks)
{
  ...
  if (xfbAddr && fbWidth && fbStride && fbHeight)
  {
    ...
    if (xfb_entry && xfb_entry->id != m_last_xfb_id)
    {
      ...
      // draw calls don't get issued until RenderImGui is called, which happens in SwapImpl.
      DrawDebugText();
      OSD::DrawMessages();
